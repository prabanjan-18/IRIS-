...
export default function NestedDropdownMenu({ onNewChat }) {
  ...
  return (
    <div className="relative" ref={menuRef} onKeyDown={handleKeyDown}>
      ...
      {actionFeedback && (
        <div className="fixed top-4 right-4 z-50 px-3 py-2 bg-ink-700 border border-frosted-500 text-mist-100 text-xs rounded-lg shadow-lg flex items-center gap-2 animate-fadeIn">
          <Check className="w-4 h-4 text-frosted-500" />
      ...
      {/* Main Dropdown Panel */}
      {isOpen && (
        <div className="absolute top-full left-0 mt-1.5 w-48 bg-ink-800 border border-ink-500 rounded-xl shadow-2xl z-50 py-1.5 animate-fadeIn">
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className={`w-full px-3 py-2 text-xs flex items-center justify-between text-mist-100 hover:bg-ink-700 hover:text-frosted-500 transition-colors ${
                    isSubmenuActive ? 'bg-ink-700 text-frosted-500' : ''
                  }`}
                >
                  <div className="flex items-center gap-2.5">
                    <SectionIcon className="w-4 h-4 text-mist-300 stroke-[1.5]" />
                    <span className="font-medium">{section.label}</span>
                  </div>
                  <ChevronRight className="w-3.5 h-3.5 text-mist-300 stroke-[1.5]" />
                </button>

                {/* Submenu Flyout to the Right */}
                {isSubmenuActive && (
                  <div 
                    className="absolute left-full top-0 ml-1 w-52 bg-ink-800 border border-ink-500 rounded-xl shadow-2xl py-1.5 z-50 animate-fadeIn"
                    ...
                    {section.items.map((item, idx) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full px-3 py-2 text-xs flex items-center justify-between text-mist-100 hover:bg-ink-600 hover:text-frosted-500 transition-colors group"
                        >
                          <div className="flex items-center gap-2">
                            {ItemIcon ? (
                              <ItemIcon className="w-3.5 h-3.5 text-mist-300 group-hover:text-frosted-500 stroke-[1.5]" />
                            ) : (
                              <div className="w-3.5" />
                            )}
                          ...
                          {item.shortcut && (
                            <span className="text-[10px] text-mist-300 font-mono">
                              {item.shortcut}
                            </span>
                          )}
                        </button>
